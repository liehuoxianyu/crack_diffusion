    ...
    if os.path.isfile(module_file_or_url):
        ...
        try:
            if cached_download is not None:
                ...
                # huggingface_hub >= 0.20 removed cached_download; use hub_download for URLs via legacy alias or manual download
                import hashlib
                from urllib.request import urlretrieve
                module_file = pretrained_model_name_or_path + ".py"
                cache_dir = Path(cache_dir) if cache_dir else Path(HF_MODULES_CACHE) / "downloads"
                cache_dir.mkdir(parents=True, exist_ok=True)
                url_hash = hashlib.sha256(github_url.encode()).hexdigest()[:16]
                resolved_module_file = str(cache_dir / f"{url_hash}_{module_file}")
                if not os.path.exists(resolved_module_file) or force_download:
                    urlretrieve(github_url, resolved_module_file)
            submodule = "git"
            module_file = pretrained_model_name_or_path + ".py"
