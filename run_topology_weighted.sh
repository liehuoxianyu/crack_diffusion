...
# ========== 数据加载配置（Topology + crack-weighted loss） ==========
...
# patch 策略：裁剪评分仍使用 binary mask，避免改变采样协议
...
export OUTPUT_DIR="/work/outputs/exp_TOPOLOGY_WEIGHTED_patch512"
...
accelerate launch train_controlnet.py \
  ...
  --crack_weighted_loss \
  --crack_loss_alpha=2.0 \
  --crack_loss_threshold=0.125 \
  --crack_loss_dilate=1 \
  --set_grads_to_none \
  --seed=42 \
